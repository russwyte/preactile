...
object AttributeConstructor:
  given Conversion[String, js.Any] = _.asInstanceOf[js.Any]

sealed trait AttributeSet:
...
trait GlobalAttributes extends AttributeSet:
  ...
  def style(styles: Declaration*) =
    val d: Dictionary[String] = js.Dictionary(styles.filter(_ != null).map(x => x.property -> x.value)*)
    Attribute("style", d)
...
trait EventListeners extends AttributeSet:
