...
object Styles:
  ...
  def selector[T <: DeclarationOrSelector](selector: String)(members: T*) = Selector(selector, members*)
  ...
  abstract class DeclarationConstructor[T](property: String):
    ...
    def customProperty(name: String, fallback: String = "") =
      val n = if name.startsWith("--") then name else s"--$name"
  ...
  object Selector:
    ...
    protected class SelectorConstructor(selector: String):
      def apply(members: DeclarationOrSelector*): Selector = Selector.apply(selector, members*)
